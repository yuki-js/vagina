...
export 'tools/base_tool.dart'
    show BaseTool, ToolExecutionResult, ToolManagerRef;
...
class ToolService {
  ...
  /// 通話開始時に呼び出される
  Future<ToolManager> createToolManager(
      {void Function()? onToolsChanged}) async {
    // 未初期化なら初期化
    ...
    for (final entry in allTools) {
      final isEnabled =
          enabledTools.isEmpty || enabledTools.contains(entry.tool.name);
      if (isEnabled) {
