...
class ToolRegistry {
  ...
  void registerTool(BaseTool tool, ToolMetadata metadata) {
    ...
    logService.info(
        _tag, 'ツール登録: ${tool.name} (${metadata.category.displayName})');
  ...
  /// カテゴリでツールをフィルタ
  List<({BaseTool tool, ToolMetadata metadata})> getToolsByCategory(
      ToolCategory category) {
    return getAllTools().where((t) => t.metadata.category == category).toList();
  }

  /// ソースでツールをフィルタ
  List<({BaseTool tool, ToolMetadata metadata})> getToolsBySource(
      ToolSource source) {
