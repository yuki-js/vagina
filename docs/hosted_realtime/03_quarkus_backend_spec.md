...
- patch 生成が 1 箇所に集約される
- `thread.patch` は fire-and-forget なライブ差分であり、`streamSeq` や revision は付けない。配信できなかった場合の回復は patch の照合ではなく、reconnect + 最新 `thread.snapshot` の取り直し一本道である
...
graph LR
    ...
    OPS --> FLUSH
    FLUSH --> OUT[VHRP thread.patch 送信 番号なし]
```
...
- **Cli-Qua 間は lossy**: モバイル client は基地局ハンドオーバ等で頻繁に瞬断・reconnect する。VHRP の resume/resync 機構 (reconnect + 最新 `thread.snapshot`) はこの境界を吸収するために存在する。
...
├── VhrpSession.java                  ; 1接続=1セッション。canonical thread を所有し、判断4の op buffer + flush を担う。配信失敗はログのみで握り潰し、能動closeはしない
├── model
...
5. `messageId` / `replyTo` 相関、`ack` / `error` 応答。live frame 配信失敗はログのみで握り潰す (能動closeしない。恒久障害は Quarkus の `@OnClose` が拾い、client の reconnect → snapshot で回復する)

`VhrpSession` は 1 WebSocket connection = 1 session として、session 固有状態 (sessionId, canonical thread への参照, pending messageId 相関) を保持する。VHRP の「active session は connection context に束縛」規定をそのまま体現する。`streamSeq` や threadRevision は持たない (回復は full snapshot 一本道)。
...
| `thread.sync.request` | (VhrpSession が直接処理) | 最新 full `thread.snapshot` |
...
sequenceDiagram
    ...
    E->>E: op を buffer して thread.patch に flush (番号なし)
...
`thread.patch` は `streamSeq` も revision も持たない。`VhrpSession` は番号を一切採番しない。

flush 単位 = Dart で `_emitThreadUpdate()` を呼んでいた境界。複数 op を 1 patch にまとめて送るだけで、版番号は付けない。届かなかった場合の回復は patch の照合ではなく、reconnect + 最新 `thread.snapshot` の取り直し一本道である (後述 resume 責務)。
...
01 の方針通り、resume/resync は adapter 境界の外 (wire + session 内部状態) で吸収する。前提として、Qua-Oai 間 (server-server link) は安定で切れにくいのに対し、Cli-Qua 間は基地局ハンドオーバ等で頻繁に reconnect しうる。したがって resume の主対象は Cli-Qua の再接続境界であり、resume が甦らせるのは canonical thread の投影 (transcript 履歴) だけで、OAI 側の生成セッションそのものの復元は狙わない。回復は単一経路に集約される。replay log も version 照合も持たない。v1 server は最低限:

- canonical thread は常にメモリ上に保持する (それ自体が「最新の正規状態」であり、過去 op を貯める必要はない)
- `thread.sync.request` には常に最新の full `thread.snapshot` を返す (mode 分岐も部分 catch-up もない)
- `session.open.resume` 付きには `session.resumed` を返すのみ。状態は自動送信せず、client が後続の `thread.sync.request` で最新 snapshot を取り直す
- live frame 配信失敗はログのみで握り潰し、server は能動closeしない。恒久的なソケット障害は framework (`@OnClose`) が検知して detach し、client の reconnect → snapshot 取り直しで回復する。一時的な失敗で生接続を切らないことで、無駄な再接続 (handshake コスト) を避ける
...
5. `VhrpSession` で op flush (番号なし)。配信失敗はログのみで握り潰す
