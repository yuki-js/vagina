...
- ⌨️ **グローバルショートカット** - 別ウィンドウ操作中でも音声入力を操作 (Windows, NEW)
...
### その他のドキュメント
- [Windows グローバルショートカット](docs/windows_global_hotkeys.md) - 別ウィンドウ操作中の音声入力操作
...
- **Instant-only** (< 30秒): 同期で結果を返す（30秒タイムアウト）
- **複数エージェント管理**: 用途別に複数のエージェントを設定可能

#### 新しいツール機能 (NEW v1.1.0)
- **end_call**: 音声エージェントがプログラマティックに通話を終了
- **say_hello_to_agent**: 音声エージェントからテキストエージェントへクエリ（同期、30秒タイムアウト）
- **list_available_agents**: 利用可能なテキストエージェントを一覧表示

#### グローバルショートカット (Windows)
- **別ウィンドウからの操作**: 音声入力の開始・送信・破棄、割り込み、ミュート切替をキーボードで実行
- **キーの自由割り当て**: 既定のキーは設定せず、ユーザーが任意の組み合わせを登録
- **競合の可視化**: 他のアプリが使用中の組み合わせは登録失敗として設定画面に表示
- 詳細: [Windows グローバルショートカット](docs/windows_global_hotkeys.md)
...
ローカルの Flutter Web では、起動中のホストとポートから `https://localhost:<port>/announcement.json` が自動的に参照されます。手動の `--dart-define` 指定は不要です。
